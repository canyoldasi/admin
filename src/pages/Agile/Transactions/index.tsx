...
const createDebouncedFormikHandlers = (
  originalHandleChange: Function,
  delay = 300
) => {
  return (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (e.target.tagName === "TEXTAREA") {
      console.log(
        `Direct textarea update for ${e.target.name}: ${e.target.value}`
      );
    ...
    const debounced = debounce(
      (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        console.log(
          `Debounced update for ${event.target.name}: ${event.target.value}`
        );
...
async function fetchTransactionData(
  ...
  try {
    ...
    console.log(
      ...
      JSON.stringify({ input: inputParams }, null, 2)
    );
...
): Promise<TransactionWithCreatedAt | null> {
  try {
...
const TransactionsContent: React.FC = () => {
  ...
  const [allTransactions, setAllTransactions] = useState<
    TransactionWithCreatedAt[]
  >([]);
  const [filteredTransactions, setFilteredTransactions] = useState<
    TransactionWithCreatedAt[]
  >([]);
  const [transaction, setTransaction] =
    useState<TransactionWithCreatedAt | null>(null);
  ...
  const [isFilteringInProgress, setIsFilteringInProgress] =
  ...
  const fetchDataWithCurrentFilters = async () => {
    ...
    if (isFilteringInProgress) {
      console.log(
        "Filtering in progress, skipping fetchDataWithCurrentFilters"
      );
      return;
    }
    ...
    try {
      ...
      const apiParams: GetTransactionsDTO = {
        ...
        statusIds: urlParams.get("status")
          ? urlParams.get("status")?.split(",")
          : null,
        typeIds: urlParams.get("typeIds")
          ? urlParams.get("typeIds")?.split(",")
          : null,
        assignedUserIds: urlParams.get("assignedUserIds")
          ? urlParams.get("assignedUserIds")?.split(",")
          : null,
        ...
        productIds: urlParams.get("productIds")
          ? urlParams.get("productIds")?.split(",")
          : null,
        cityIds: urlParams.get("cityIds")
          ? urlParams.get("cityIds")?.split(",")
          : null,
        channelIds: urlParams.get("channelIds")
          ? urlParams.get("channelIds")?.split(",")
          : null,
      ...
      if (result) {
        ...
        if (minAmount !== null || maxAmount !== null) {
          ...
          console.log(
            "Tutar filtresi sonrası kalan işlemler:",
        ...
        console.log(
          `Sayfalama: Toplam ${result.itemCount} işlem, Sayfa ${
            pageIndex + 1
          }/${result.pageCount}`
        );
      } else {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const apiParams: GetTransactionsDTO = {
        ...
        orderDirection: (
          urlParams.get("orderDirection") || orderDirection
  ...
  const updateUrlParams = (
    ...
    if (typeIds && typeIds.length > 0)
      searchParams.set("typeIds", typeIds.join(","));
    if (assignedUserIds && assignedUserIds.length > 0)
      searchParams.set("assignedUserIds", assignedUserIds.join(","));
    if (productIds && productIds.length > 0)
      searchParams.set("productIds", productIds.join(","));
    if (cityIds && cityIds.length > 0)
      searchParams.set("cityIds", cityIds.join(","));
    if (channelIds && channelIds.length > 0)
      searchParams.set("channelIds", channelIds.join(","));
    ...
    if (minAmount !== undefined && minAmount !== null)
      searchParams.set("minAmount", minAmount.toString());
    if (maxAmount !== undefined && maxAmount !== null)
      searchParams.set("maxAmount", maxAmount.toString());

    // Log URL parameters
    console.log(
      "URL parametreleri (updateUrlParams):",
  ...
  const handleSort = (key: string) => {
    ...
    if (key === "products") {
      ...
      setSortConfig((prevConfig) => {
        ...
        const sortedTransactions = [...filteredTransactions].sort((a, b) => {
          ...
          const productNamesA = productsA
            .map((p: any) => p.product?.name || "")
            .join(", ");
          const productNamesB = productsB
            .map((p: any) => p.product?.name || "")
            .join(", ");
    ...
    setSortConfig((prevConfig) => {
      ...
      console.log(
        "Sıralama değişimi API parametreleri:",
      ...
      fetchTransactionData(apiParams)
        .then((result) => {
          if (result) {
            ...
            setPageCount(result.pageCount);
            console.log(
              `Sıralama değişimi sonrası: Toplam ${
                result.itemCount
              } işlem, Sayfa ${pageIndex + 1}/${result.pageCount}`
            );
          } else {
            // No results
            setAllTransactions([]);
  ...
  const handlePageChange = (page: number) => {
    ...
    const apiParams: GetTransactionsDTO = {
      ...
      statusIds: urlParams.get("statusIds")
        ? urlParams.get("statusIds")?.split(",")
        : null,
      typeIds: urlParams.get("typeIds")
        ? urlParams.get("typeIds")?.split(",")
        : null,
    ...
    console.log(
      "Sayfa Değişimi API parametreleri:",
      JSON.stringify(apiParams, null, 2)
    );
    ...
    fetchTransactionData(apiParams)
      .then((result) => {
        if (result) {
          ...
          setPageCount(result.pageCount);
          console.log(
            `Sayfalama: Toplam ${result.itemCount} işlem, Sayfa ${page + 1}/${
              result.pageCount
            }`
          );
        } else {
  ...
  const handlePageSizeChange = (size: number) => {
    ...
    console.log(
      "Sayfa Boyutu Değişimi API parametreleri:",
      JSON.stringify(apiParams, null, 2)
    );
    ...
    fetchTransactionData(apiParams)
      .then((result) => {
        if (result) {
          ...
          setPageCount(result.pageCount);
          console.log(
            `Sayfalama: Toplam ${result.itemCount} işlem, Sayfa 1/${result.pageCount}`
          );
        } else {
          // Sonuç bulunamadı
          setAllTransactions([]);
  ...
  const safelyUpdateFormField = (fieldName: string, value: any) => {
    ...
    console.log(
      `Updated ${fieldName} to ${value}. Form values:`,
      validation.values
    );
  };
  ...
  const validation = useFormik({
    ...
    initialValues: {
      ...
      // Add products field
      products:
        transaction?.transactionProducts?.map((p: any) => ({
          value: p.product.id,
      ...
      successDate:
        (transaction && transaction.successDate) ||
      ...
      if (isEdit) {
        ...
        try {
          ...
          const input: any = {
            ...
            channelId: nullIfEmpty(values.channelId),
            transactionDate:
              nullIfEmpty(values.transactionDate) ||
              moment().format("YYYY-MM-DD"),
  ...
  const debouncedHandleChange = createDebouncedFormikHandlers(
  ...
  const [getCounties, { loading: countiesLoading }] = useLazyQuery(
    GET_COUNTIES,
    {
      context: getAuthorizationLink(),
      fetchPolicy: "network-only",
  ...
  const [getDistricts, { loading: districtsLoading }] = useLazyQuery(
    GET_DISTRICTS,
    {
      context: getAuthorizationLink(),
      fetchPolicy: "network-only",
  ...
  const { loading: rolesLoading } = useQuery(GET_TRANSACTION_TYPES, {
    ...
    onCompleted: (data) => {
      if (data && data.getTransactionTypesLookup) {
        setRoleOptions(
          data.getTransactionTypesLookup.map((role: any) => ({
            value: role.id,
            label: role.name,
          }))
        );
  ...
  const { loading: typesLoading } = useQuery(GET_TRANSACTION_TYPES, {
    ...
    onCompleted: (data) => {
      if (data && data.getTransactionTypesLookup) {
        setTypeOptions(
          data.getTransactionTypesLookup.map((type: any) => ({
            value: type.id,
            label: type.name,
          }))
        );
    ...
    onCompleted: (data) => {
      if (data && data.getTransactionStatusesLookup) {
        const options = data.getTransactionStatusesLookup.map(
          (status: any) => ({ value: status.id, label: status.name })
        );
    ...
    try {
      ...
      if (
        ...
        if (accounts.length > 0) {
          ...
          console.log(
            `Accounts loaded successfully: ${options.length} accounts found`
          );
          ...
          console.warn(
            "Using empty account list - no accounts available from API"
          );
      ...
      // Type assertion to 'any' to access Apollo error properties
  ...
  const handleTransactionClick = useCallback(
    async (selectedTransaction: any) => {
      try {
        ...
        console.log(
          "Fetching complete transaction data for editing, ID:",
        ...
        console.log("Geographic data details:", {
          county: completeTransaction.county
            ? {
                id: completeTransaction.county.id,
                name: completeTransaction.county.name,
              }
            : null,
          district: completeTransaction.district
            ? {
                id: completeTransaction.district.id,
                name: completeTransaction.district.name,
              }
            : null,
        });
        ...
        const processedProducts =
          completeTransaction.transactionProducts?.map((product: any) => {
        ...
        const calculatedAmount = processedProducts.reduce(
        ...
        const formValues = {
          ...
          products:
            updatedTransaction.transactionProducts?.map((p: any) => ({
              value: p.product.id,
              label: p.product.name,
            })) || [],
          date: updatedTransaction.createdAt
            ? moment(updatedTransaction.createdAt).format("DD.MM.YYYY")
            : moment().format("DD.MM.YYYY"),
          transactionDate: updatedTransaction.transactionDate
            ? moment(updatedTransaction.transactionDate).format("YYYY-MM-DD")
            : moment().format("YYYY-MM-DD"),
        ...
        if (updatedTransaction.account && updatedTransaction.account.id) {
          ...
          const accountExists = accountOptions.some(
        ...
        console.log(
          "Account ID in form after setting:",
        ...
        if (formValues.country) {
          try {
            ...
            if (!cachedCities || !cachedCities.getCities) {
              console.log(
                `Loading cities for country ID: ${formValues.country}`
              );
              ...
              console.log(
                ...
                cityResult?.data?.getCities?.length || 0,
                "cities"
              );
            } else {
              console.log(
                "Using cached cities:",
                cachedCities.getCities.length
              );
            }
            ...
            if (formValues.city) {
              ...
              if (!cachedCounties || !cachedCounties.getCounties) {
                ...
                console.log(
                  ...
                  countyResult?.data?.getCounties?.length || 0,
                  "counties"
                );
              } else {
                console.log(
                  "Using cached counties:",
                  cachedCounties.getCounties.length
                );
              }
              ...
              if (formValues.district) {
                ...
                if (!cachedDistricts || !cachedDistricts.getDistricts) {
                  console.log(
                    `Loading districts for county ID: ${formValues.district}`
                  );
                  ...
                  console.log(
                    ...
                    districtResult?.data?.getDistricts?.length || 0,
                    "districts"
                  );
                } else {
                  console.log(
                    "Using cached districts:",
                    cachedDistricts.getDistricts.length
                  );
                }
              }
            }
      ...
      try {
        ...
        if (result && result.data && result.data.getTransaction) {
          ...
          localStorage.setItem(
            `transaction_${selectedTransaction.id}`,
            JSON.stringify(transactionData)
          );
          console.log(
            "İşlem detayları localStorage'a kaydedildi:",
            transactionData
          );
        } else {
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (selectedRecordForDelete && selectedRecordForDelete.id) {
      try {
        ...
        console.log(
          "Silinen işlem ID (tip):",
          typeof selectedRecordForDelete.id
        );
        console.log("Silinen işlem ID (değer):", selectedRecordForDelete.id);

        // ID null veya undefined mi diye kontrol et
        if (
          selectedRecordForDelete.id === null ||
          selectedRecordForDelete.id === undefined
        ) {
        ...
        console.log(
          "DELETE_TRANSACTION mutation yapısı:",
          DELETE_TRANSACTION.loc?.source?.body
        );
        ...
        if (error.graphQLErrors && error.graphQLErrors.length > 0) {
          ...
          toast.error(
            "Sunucu bağlantı hatası. Lütfen ağ bağlantınızı kontrol edin."
          );
        } else {
          toast.error("İşlem silinirken bir hata oluştu.");
        }
  ...
  const handleFilterApply = async (
    ...
    try {
      ...
      const country = filters.country ? filters.country.value : undefined;
      ...
      console.log("API parametreleri:", JSON.stringify(apiParams, null, 2));
      console.log(
        "API'ye gönderilen sayfa boyutu (pageSize):",
        apiParams.pageSize
      );
      console.log(
        "API'ye gönderilen sayfa indeksi (pageIndex):",
        apiParams.pageIndex
      );
        ...
        if (isEdit) {
          ...
          const input: any = {
            ...
            ...(validation.values.neighborhood
              ? { districtId: validation.values.neighborhood }
              : {}),
          ...
          const input: any = {
            ...
            transactionDate:
              nullIfEmpty(validation.values.transactionDate) ||
              moment().format("YYYY-MM-DD"),
          ...
          console.log("Channel details:", {
            ...
            channelOptions: channelOptions.map((opt) => ({
              value: opt.value,
              label: opt.label,
            })),
          });
      ...
      try {
        ...
        try {
          if (transaction.country?.id) {
            console.log(
              `Pre-loading cities for country: ${transaction.country.id}`
            );
            ...
            if (!cachedCities) {
              ...
              console.log(
                "Cities loaded successfully:",
                citiesResult?.data?.getCities?.length || 0,
                "cities"
              );
            } else {
            ...
            if (transaction.city?.id) {
              console.log(
                `Pre-loading counties for city: ${transaction.city.id}`
              );
              ...
              if (!cachedCounties) {
                ...
                console.log(
                  "Counties loaded successfully:",
                  countiesResult?.data?.getCounties?.length || 0,
                  "counties"
                );
              } else {
              ...
              if (countyId) {
                ...
                if (!cachedDistricts) {
                  ...
                  console.log(
                    "Districts loaded successfully:",
                    districtsResult?.data?.getDistricts?.length || 0,
                    "districts"
                  );
                } else {
          ...
          const formValues = {
            ...
            successDate:
              transaction.successDate || moment().format("YYYY-MM-DD HH:mm"),
            successNote: transaction.successNote || "",
            transactionNote: transaction.note || "",
            cancelDate:
              transaction.cancelDate || moment().format("YYYY-MM-DD HH:mm"),
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: (cell: any) => {
          return cell?.row.original.transactionDate
            ? moment(cell?.row.original.transactionDate).format(
                "DD.MM.YYYY HH:mm"
              )
            : "-";
        },
      ...
      },
      {
        header: (
          <span
            style={{ cursor: "pointer" }}
            onClick={() => handleSort("assignedUser.fullName")}
          >
            Atanan Kullanıcı{" "}
            {sortConfig?.key === "assignedUser.fullName"
              ? sortConfig.direction === "asc"
                ? "▲"
                : "▼"
              : ""}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (!transaction || !transaction.id) {
                    toast.error(
                      "Geçersiz işlem verisi. Silme işlemi yapılamaz."
                    );
                    return;
                  }
  ...
  useEffect(() => {
    if (isInfoDetails) {
      console.log(
        "Filtre paneli açıldı, URL parametreleri kontrol ediliyor..."
      );
    ...
    const loadUserOptions = async () => {
      try {
        ...
        if (data && data.getUsersLookup && data.getUsersLookup.items) {
          ...
          console.log(
            `Users loaded successfully: ${options.length} users found`
          );
        } else {
      ...
      Promise.all([
        ...
        .then(([channelsResult, accountsResult]) => {
          ...
          if (channelsResult.data?.getChannelsLookup) {
            const channelOpts = channelsResult.data.getChannelsLookup.map(
              (channel: any) => ({
                value: channel.id,
                label: channel.name,
              })
          ...
          if (accountsResult.data?.getAccounts?.items) {
            const accountOpts = accountsResult.data.getAccounts.items.map(
              (account: any) => ({
                value: account.id,
                label: account.name,
              })
    ...
    if (match && match[1]) {
      ...
      const fetchTransactionForEdit = async () => {
        try {
          ...
          if (data && data.getTransaction) {
            console.log(
              "Transaction data loaded for edit:",
              data.getTransaction
            );
  ...
  useEffect(() => {
    ...
    if (
      previousModalState.current.isEdit ||
  ...
  const MemoizedGrid = memo(
    ({
      ...
      return (
        <TableContainer
          ...
          tableClass={
            tableClass ||
            (loading ? "align-middle table-loading" : "align-middle")
          }
    ...
    (prevProps, nextProps) => {
      ...
      const pageSizeChanged =
        prevProps.customPageSize !== nextProps.customPageSize;
                ...
                <CardBody className="pt-3">
                  <TransactionFilter
                    ...
                    onFilterApply={async (filters: TransactionFilterState) => {
                      try {
                        console.log(
                          "Applying filters from index.tsx:",
                        ...
                        if (
                          ...
                          const graphQLError = error.graphQLErrors[0];
                          toast.error(
                            `Filtreleme hatası: ${graphQLError.message}`
                          );
                        } else if (error.networkError) {
                          toast.error(
                            `Ağ hatası: ${
